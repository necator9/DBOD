...
void FeatureExtraxtor::find_basic_params(cv::Mat &features){
    // Compose matrix from coordinates of bounding rectangles for convenience
    cv::Mat boundRect_arr = cv::Mat_<double>(contours.size(), 6);
    cv::Mat ca_px = cv::Mat_<double>(contours.size(), 1);
    for(auto r = 0; r < boundRect_arr.rows; r++){
        double* ptr_br = boundRect_arr.ptr<double>(r);
        double* ptr_ca = ca_px.ptr<double>(r);
        ptr_ca[0] = ca[r];
        ptr_br[0] = boundRect[r].x;
        ptr_br[1] = boundRect[r].y;
        ptr_br[2] = boundRect[r].br().x;
        ptr_br[3] = boundRect[r].br().y;
        ptr_br[4] = boundRect[r].width;
        ptr_br[5] = boundRect[r].height;
    }

    // Important! Reverse the y coordinates of bound. rect. along y axis before transformations
    cv::Mat px_y_bottom_top = cv::Mat_<double>(contours.size(), 2);
    px_y_bottom_top.col(0) = img_res.height - boundRect_arr.col(3);
    px_y_bottom_top.col(1) = img_res.height - boundRect_arr.col(1);

    cv::Mat y_bottom_top_to_hor = cv::Mat_<double>(contours.size(), 2);
    ...
    cv::Mat px_x_l = cv::Mat_<double>::ones(contours.size(), 3);
    boundRect_arr.col(0).copyTo(px_x_l.col(0));
    px_y_bottom_top.col(0).copyTo(px_x_l.col(1));

    cv::Mat px_x_r = cv::Mat_<double>::ones(contours.size(), 3);
...
void FeatureExtraxtor::estimate_3d_coordinates(cv::Mat &rw_coords, const cv::Mat &px_x_lr, const cv::Mat &rw_distance) {
    ...
    cv::Mat z_cam_coords = cv::Mat_<double>(contours.size(), 1); 
    z_cam_coords = cam_h * sin(rx_rad) + rw_distance * cos(rx_rad);
    cv::Mat z_cam_coords_2x = cv::Mat_<double>(contours.size() * 2, 1);
    cv::vconcat(z_cam_coords, z_cam_coords, z_cam_coords_2x);

    cv::Mat cam_xlr_yb_h = cv::Mat_<double>(contours.size() * 2, 3);
